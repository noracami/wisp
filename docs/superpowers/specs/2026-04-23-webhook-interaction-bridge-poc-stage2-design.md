...
1. `/tpp-setup` → 取得授權連結
2. 瀏覽器開連結，授權到一個**你有 Manage Webhooks 權限的** guild channel
3. 授權頁載入時**順手截圖記錄**：
   - Channel selector 列出了哪些選項（只 guild channel？有 DM？）— Q9
   - 若有多 guild，selector 是否能跨 guild 切換？— Q6
4. 確認 OAuth callback 成功（瀏覽器看到成功頁 + server log `tpp_poc.oauth.callback`）
5. 檢查 DB：`SELECT * FROM tpp_webhooks` 應有一筆
6. `/tpp-ping` → 看 `tpp_poc.ping.send.done` status = 200 / 204
...
Discord OAuth `webhook.incoming` 的授權頁預期**只能選 guild channel**（由 Q9 在 Experiment A 驗證）。若 Q9 結果顯示 DM 選項意外可選，再回來補跑。
...
**Nice-to-have（影響 Stage 3 UX 設計，不影響 go/no-go）**：
- Q6 / Q7 / Q8 / Q9 記錄結果即可
